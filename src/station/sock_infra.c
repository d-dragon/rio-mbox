...
int openStreamSocket() {
	...
	if (stream_sock_fd < 0) {
		...
		printf( "Open Sream socket success\n");
	}
	...
	if (ret < 0) {
		printf("call listen() error\n");
		return SOCK_ERROR;
	} else {
		printf( "TCP socket is listening incoming connection at %s\n",
				interface_addr);
	}
	return SOCK_SUCCESS;
}
...
int openDatagramSocket() {
	...
	int ret;
	broadcast_enable = 1;
	printf("Create UDP Broadcast socket......\n");

	datagram_sock_fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (datagram_sock_fd < 0) {
		printf("Open UDP Broadcast socket failed\n");
		return SOCK_ERROR;
	} else {
		printf( "Open UDP Broadcast socket success\n");
	}
	...
	printf( "Config UDP Server sock address success\n");
	...
	if (ret < 0) {
		printf("bind address to UDP server socket failed!\n");
		return SOCK_ERROR;
	} else {
		printf( "bind address to UDP server socket success!\n");
	}
	return SOCK_SUCCESS;
}
...
int openMulRecvSocket() {
	...
	if (mul_fd < 0) {
		...
		printf( " open multicast socket success\n");
	}
	...
	if (setsockopt(mul_fd, SOL_SOCKET, SO_REUSEADDR, (char *) &reuse,
			sizeof(reuse)) < 0) {
		printf( "setting REUSEADDR failed\n");
		close(mul_fd);
		return SOCK_ERROR;
	}
	...
	if (bind(mul_fd, (struct sockaddr*) &mul_sock, sizeof(mul_sock))) {
		...
		printf( "binding mul socket failed\n");
		close(mul_fd);
		return SOCK_ERROR;
	}
...
int connecttoStreamSocket(char *addr, char *port) {
	...
	if (connect(sd_sock, (struct sockaddr*) &serv_addr, sizeof(serv_addr))
		...
		if (errno == EINPROGRESS) {
			...
			printf( "connect to station failed");
			return SOCK_ERROR;
		}
	}
